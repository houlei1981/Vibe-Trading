...
- **2026-06-17** 🛠️ **Alpha Zoo 中文化 + Windows 一键启动**：Alpha Zoo 浏览页、详情页、智能体示例与因子元数据继续补齐中英双语切换，主题 / universe / 说明文案在中文界面下会自动翻译；因子列表改为每页 20 条的分页模式，支持页码跳转，并在返回详情页后保留最近浏览页。新增根目录 `start-vibe-trading.bat`，可在 Windows 下双击后一键启动后端 + 前端并自动打开浏览器，命令行窗口保持打开便于排错。
