...
- **2026-06-17** 🛠️ **Alpha Zoo bilingual UI + Windows one-click launcher**: Alpha Zoo browsing, detail pages, agent examples, and factor metadata now continue the Chinese/English toggle across the main surfaces; theme / universe / explanatory copy translate in Chinese mode. The factor list now uses 20-per-page pagination with page numbers and direct jump input, and returning from a factor detail preserves the last browsed page. Added `start-vibe-trading.bat` at the repo root so Windows users can double-click to start backend + frontend, auto-open the browser, and keep both console windows visible for debugging.
